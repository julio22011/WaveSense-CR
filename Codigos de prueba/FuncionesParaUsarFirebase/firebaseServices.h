...
void sendDataRTDB(){  // recibir dos vectores, uno con los nombres y otro con los valores

  if (Firebase.ready()){

    timeAge++; // incrementa la edad del tiempo
    
    // Obtener el timestamp para ordenar la ruta y guardar hora en el rtc
    //----------------------------------------------
    if (!gotTimestamp && timeAge < maxTimeAge)Serial.printf("Set timestamp... %s\n", Firebase.RTDB.setTimestamp(&fbdo, "/deviceReadings/esp32-x/lastTime/timestamp") ? "ok" : fbdo.errorReason().c_str());
    ...
    //-----------------------------------------------
    if (gotTimestamp){  // continua una ver se ha actualizado la hora
      ...
      String ruta = "/deviceReadings/esp32-x/readings" + rutaTime;  // agregarle a la ruta el orden de la fecha
      ...
      json.set("temp", 23*random(0,5)); // en lugar de "Data" se puede usar el timestamp del rtc
      json.set("ph", 6.7*random(0,5)); // en lugar de "Data" se puede usar el timestamp del rtc
      json.set("cond", 91*random(0,5)); // en lugar de "Data" se puede usar el timestamp del rtc
      json.set("tds", 45*random(0,5)); // en lugar de "Data" se puede usar el timestamp del rtc
