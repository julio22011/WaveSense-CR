...
static lv_obj_t *bodyScreen; //añadido
static lv_obj_t *ejemplolabel; //añadido
...
//static void list_event_handler(lv_event_t *e); // eliminado por repetido
...
void saveWIFICredentialEEPROM(int flag, String ssidpw) {
  ...
  // Julio: Guardar credenciales en preferencias
  preferences.putString("ssid", ssidName.c_str());              //"KONOHA";        // char*
  preferences.putString("password", ssidPW.c_str());     //"familiars58";  // char*

  // Julio: enviar credenciales al esp32 de la red
  Serial2.printf("Credentials: %s:::%s\n", ssidName, ssidPW.c_str());
}
...
void loadWIFICredentialEEPROM() {
  ...
  if (wifiFlag == 1 && wifiCredential.length() != 0 && wifiCredential.indexOf(" ") != -1) {
    ...
    if (sscanf(wifiCredential.c_str(), "%s %s", preSSIDName, preSSIDPw) == 2) {
      ...
      popupMsgBox("Bienvenido!", "Intentando conectar a la red previamente conectada.");
      ssidName = String(preSSIDName);
      ssidPW = String(preSSIDPw);

      // Julio: Actulizar los datos de red por defecto
      //ssid = ssidName.c_str();
...
static void buildStatusBar() {
  ...
  lv_label_set_text(timeLabel, "WiFi no conectado!    " LV_SYMBOL_CLOSE);
  ...
  batteryBtn = lv_btn_create(statusBar);
  lv_obj_set_size(batteryBtn, 30, 30);
  lv_obj_align(batteryBtn, LV_ALIGN_RIGHT_MID, -35, 0);

  lv_obj_add_event_cb(batteryBtn, btn_event_cb, LV_EVENT_ALL, NULL); // la funcion callback es la misma
  lv_obj_t *label2 = lv_label_create(batteryBtn); /*Add a label to the button*/
  lv_label_set_text(label2, LV_SYMBOL_BATTERY_FULL);  /*Set the labels text*/
...
static void btn_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED) {
    ...
    if (btn == settingBtn) {
      ...
      popupMsgBox("Conectando!", "Intantando conectar con la red seleccionada.");
    ...
    if (btn == settingWiFiSwitch) {
      ...
      if (lv_obj_has_state(btn, LV_STATE_CHECKED)) {
        ...
        if (ntScanTaskHandler == NULL) {
          ...
          networkScanner();
          timer2 = lv_timer_create(timerForNetwork, 1000, wfList);
          lv_list_add_text(wfList, "WiFi: Buscando redes...");
        }
...
static void timerForNetwork(lv_timer_t *timer) { 
  ...
  switch (networkStatus) {
    ...
    case NETWORK_CONNECTED_POPUP:
      popupMsgBox("WiFi Conectado!", "Ahora se guardaran estas credenciales.");
    ...
    case NETWORK_CONNECT_FAILED:
      networkStatus = NETWORK_SEARCHING;
      popupMsgBox("Oops!", "Por favor, verifica la contraseña e intenta de nuevo.");
      break;
...
static void buildSettings() {
  ...
  lv_label_set_text(settinglabel, "Configuracion " LV_SYMBOL_SETTINGS);
  ...
  if (code == LV_EVENT_CLICKED) {
    ...
    for (int i = 0; i < selectedItem.length() - 1; i++) {
      if (selectedItem.substring(i, i + 2) == " (") {
        ...
        lv_label_set_text_fmt(mboxTitle, "WiFi seleccionado: %s", ssidName);
...
static void buildPWMsgBox() {
  ...
  lv_label_set_text(mboxTitle, "WiFi seleccionado: ThatProject");
  ...
  lv_textarea_set_placeholder_text(mboxPassword, "Contraseña?");
